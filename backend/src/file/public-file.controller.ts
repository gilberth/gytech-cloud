...
export class PublicFileController {
  ...
  async getFile(
    ...
    try {
      ...
      if (
        file.metaData.share?.emailNotification &&
        file.metaData.share?.creator?.email
      ) {
        const actionType =
          download === "false" ? "File Preview" : "File Download";
        const details = `File: ${file.metaData.name} (Size: ${file.metaData.size})\nToken: ${token}`;

        // Fire and forget - don't await to avoid blocking response
        this.emailService
          .sendShareAccessNotification(
            file.metaData.share.creator.email,
          ...
          .catch((e) =>
            console.error("Failed to send share notification email", e),
          );
      }
