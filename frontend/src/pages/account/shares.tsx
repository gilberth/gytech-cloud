...
const MyShares = () => {
  ...
  const getShareStatus = (share: MyShare) => {
    ...
    if (expiration.unix() === 0) {
      return {
        status: "active",
        label: t("account.shares.table.expiry-never"),
        color: "blue",
      };
    }
    ...
    if (expiration.isBefore(now)) {
      return {
        status: "expired",
        label: t("account.shares.status.expired"),
        color: "red",
      };
    }
    ...
    if (expiration.diff(now, "days") <= 7) {
      return {
        status: "expiring",
        label: t("account.shares.status.expiring"),
        color: "yellow",
      };
    }
    ...
    if (share.security.maxViews && share.views >= share.security.maxViews) {
      return {
        status: "view-limit",
        label: t("account.shares.status.view-limit"),
        color: "orange",
      };
    }
    ...
    return {
      status: "active",
      label: t("account.shares.status.active"),
      color: "green",
    };
  };
      ...
      <Stack spacing="sm">
        <Group position="apart" align="flex-start">
          <Group spacing="sm" style={{ flex: 1 }}>
            ...
            <Box>{renderFileThumbnail(share.files, share.id)}</Box>
          </Group>
        </Group>
        ...
        <Group position="center" spacing="xs">
          <ActionIcon
            ...
            size="sm"
            onClick={() => {
              showShareEditModal(modals, share, () => {
            ...
            share.files.some((file: any) =>
              ...
              <ActionIcon
                ...
                onClick={() => {
                  ...
                  const previewableFile = share.files.find((file: any) =>
                    shareService.doesFileSupportPreview(file.name),
                  );
                  if (previewableFile) {
  ...
  const handleSelectAll = () => {
    if (selectedShares.size === filteredShares?.length) {
      ...
      setSelectedShares(
        new Set(filteredShares?.map((share) => share.id) || []),
      );
    }
  };
  ...
  const handleBulkDelete = () => {
    ...
    modals.openConfirmModal({
      title: t("account.shares.bulk.confirm-delete.title", {
        count: selectedCount,
      }),
      children: (
        <Text size="sm">
          {t("account.shares.bulk.confirm-delete.description", {
            count: selectedCount,
          })}
        </Text>
      ),
      ...
      onConfirm: async () => {
        try {
          ...
          await Promise.all(
            Array.from(selectedShares).map((shareId) =>
          ...
          // Update the state
          const updatedShares =
            shares?.filter((share) => !selectedShares.has(share.id)) || [];
          setShares(updatedShares);
          setFilteredShares(updatedShares);
          setSelectedShares(new Set());

          toast.success(
            `${selectedCount} archivos compartidos eliminados correctamente`,
          );
        } catch (error) {
              ...
              {selectedShares.size > 0 && (
                <>
                  <Text size="sm" color="dimmed">
                    {t("account.shares.bulk.selected-count", {
                      count: selectedShares.size,
                    })}
                  </Text>
                  <Button
                    variant="light"
            ...
            <Center style={{ height: "200px" }}>
              <Stack align="center" spacing={10}>
                <Text size="lg" color="dimmed">
                  {searchQuery
                    ? t("account.shares.search.no-results")
                          ...
                          {share.files &&
                            ...
                            share.files.some((file: any) =>
                              ...
                              <ActionIcon
                                ...
                                onClick={() => {
                                  // Find the first previewable file
                                  const previewableFile = share.files.find(
                                    (file: any) =>
                                      shareService.doesFileSupportPreview(
                                        file.name,
                                      ),
                                  );
                                  if (previewableFile) {
